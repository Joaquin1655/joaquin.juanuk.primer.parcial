...
function sumarAlCarrito(id) {
    const todosLosProductos = hamburguesas.concat(bebidas, tragos);

    const productoEncontrado = todosLosProductos.find(function(p) {
        return p.id === id;
    });
    ...
    alert(`Un/una: ${productoEncontrado.nombre} fue agregado al carrito`)

    console.log("carrito antes de guardar en el LocalStorage: ", carrito)
...
function restarDelCarrito(id) {
    ...
    const indexEncarrito = carrito.findIndex(function(p) {
        return p.id === id
    });

    if (indexEncarrito === -1) {
        alert(`No hay más de este elemento en el carrito en el carrito`) // terminar despues
        return;
    }

    if(carrito[indexEncarrito].cantidad > 1) {
        carrito[indexEncarrito].cantidad -=1;
        alert(`Un/una: ${carrito[indexEncarrito].nombre} fue eliminado`)
    ...
    localStorage.setItem("carrito", JSON.stringify(carrito))
    console.log("carrito despues de guardar en el LocalStorage: ", carrito)
}

function mostrarProductos() {

    const contenedorHamburgesas = document.getElementById("listado-hamburguesas")
    const contenedorBebidas = document.getElementById("listado-bebidas")
    const contenedorTragos = document.getElementById("listado-tragos")

    let htmlGenerado = '';

    hamburguesas.forEach(function(producto) {
        htmlGenerado += `
        <li class="li-hamburguesa">
                    <img class="img-producto" src="./assets/hamburguesas/burga-${producto.id}.jpg" alt="hamburguesa-5">
                    <div>
    ...
    contenedorHamburgesas.innerHTML = htmlGenerado;
...
let htmlTragos = '';
        tragos.forEach(function(producto) {
        htmlTragos += `
        <li class= "li-tragos">
            <img class="img-producto" src=".${producto.img}" alt="${producto.nombre}">
            <div>
...
contenedorTragos.innerHTML = htmlTragos;
